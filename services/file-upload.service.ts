import { PrismaClient, FileUpload, UploadStatus } from '@prisma/client';
...
export class FileUploadService {
   ...
   * Update file status
   */
  async updateFileStatus(fileId: string, status: UploadStatus, errorMessage?: string): Promise<FileUpload> {
    return this.prisma.fileUpload.update({
      where: { id: fileId },
      data: {
        status,
        errorMessage,
        ...(status === UploadStatus.COMPLETED || status === UploadStatus.FAILED ? { processedAt: new Date() } : {})
      }
    });
  }
