...
export class ImportFileService {
   ...
   */
  async processInstagramLeadImport(
    jobId: string,
    userId: string
  ): Promise<void> {
    // Get the job
    ...
    try {
      // Update file status
      await this.fileUploadService.updateFileStatus(
        job.fileUploadId,
        "PROCESSING"
      )
      ...
      if (!validation.isValid) {
        throw new Error(
          `Invalid CSV format: Missing columns: ${validation.missingColumns.join(", ")}`
        )
      }
      ...
      await this.logImportJob(
        jobId,
        "INFO",
        `Found ${rows.length} leads in CSV`
      )
      ...
      for (let i = 0; i < totalLeads; i += chunkSize) {
        ...
        const progress = Math.min(
          20 + Math.floor(((i + chunk.length) / totalLeads) * 70),
          90
        )
      ...
      if (updatedJob?.batchId) {
        await this.instagramLeadService.updateBatchStatus(
          updatedJob.batchId,
          "FAILED",
          errorMessage
        )
